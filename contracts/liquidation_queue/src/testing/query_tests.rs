...
use cosmwasm_std::{from_binary, Coin, Decimal, Decimal256, Uint128, Uint256};
...
fn query_liquidation_amount() {
    ...
    // fee_deductor = 0.931095 --> A bit higher, because there are no tax fees
    ...
    assert_eq!(
        ...
        LiquidationAmountResponse {
            collaterals: vec![
                ("token0000".to_string(), Uint256::from(354420u64)),
                ("token0001".to_string(), Uint256::from(708843u64)),
                ("token0002".to_string(), Uint256::from(1063265u64)),
            ],
        }
    );
}
