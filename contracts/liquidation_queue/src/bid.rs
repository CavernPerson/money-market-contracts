...
pub fn retract_bid(
    ...
    let withdraw_amount: Uint256 = if bid.wait_end.is_some() {
        ...
        if waiting_withdraw_amount == bid.amount {
            ...
            bid.amount -= waiting_withdraw_amount;
            store_bid(deps.storage, bid.idx, &bid)?;
        }
        ...
        bid_pool.total_bid_amount -= withdraw_amount;
    ...
    if !withdraw_amount.is_zero() {
        messages.push(CosmosMsg::Bank(BankMsg::Send {
            ...
            amount: vec![Coin {
                ...
                amount: withdraw_amount.try_into()?,
            }],
        }));
    }
...
pub fn execute_liquidation(
    ...
    if !liquidator_fee.is_zero() {
        messages.push(CosmosMsg::Bank(BankMsg::Send {
            ...
            amount: vec![Coin {
                denom: config.stable_denom.clone(),
                amount: liquidator_fee.try_into()?,
            }],
        }));
    }
...
fn execute_pool_liquidation(
    ...
    bid_pool.total_bid_amount -= pool_required_stable;
    ...
    if expense_per_bid == Decimal256::one() {
        ...
        bid_pool.product_snapshot = if new_product < Decimal256::raw(1_000_000_000u128) {
            bid_pool.current_scale += Uint128::from(1u128);

            bid_pool.product_snapshot * Decimal256::from_ratio(1_000_000_000u128, 1u128) * product
        } else {
            new_product
        };
    }
...
pub(crate) fn calculate_remaining_bid(
    ...
    let bid_residue = remaining_bid_dec - Decimal256::from_ratio(remaining_bid, 1u128);
...
pub(crate) fn calculate_liquidated_collateral(
    ...
    let liquidated_collateral_dec = Decimal256::from_ratio(bid.amount, 1u128)
    ...
    let residue_collateral =
        liquidated_collateral_dec - Decimal256::from_ratio(liquidated_collateral, 1u128);
...
fn claim_col_residue(bid_pool: &mut BidPool) -> Uint256 {
    ...
    if !claimable.is_zero() {
        bid_pool.residue_collateral -= Decimal256::from_ratio(claimable, 1u128);
    }
    claimable
}
