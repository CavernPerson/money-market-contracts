use cosmwasm_std::{OverflowError, StdError, ConversionOverflowError, Uint128};
use thiserror::Error;
...
pub enum ContractError {
    ...
    LiquidationAmountExceedsLocked(Uint128),

    #[error("Lock amount cannot excceed the user's spendable amount: {0}")]
    LockAmountExceedsSpendable(Uint128),
    ...
    UnlockAmountExceedsLocked(Uint128),

    #[error("Withdraw amount cannot exceed the user's spendable amount: {0}")]
    WithdrawAmountExceedsSpendable(Uint128),
}
